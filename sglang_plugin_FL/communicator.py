...
class CommunicatorFL:
    ...
    def __init__(
        ...
        # Determine backend
        from sglang_plugin_FL.platform import PlatformFL
        try:
            platform = PlatformFL()
        ...
        if self._dist_backend == "flagcx" and world_size > 1:
            try:
                from sglang_plugin_FL.flagcx_communicator import FlagCXCommunicator
                self._flagcx_comm = FlagCXCommunicator(
                    group=cpu_group,
                    device=device,
                )
                if not self._flagcx_comm.available:
                    logger.warning("FlagCX communicator init failed, falling back to torch.distributed")
                    self._flagcx_comm = None
            except Exception as e:
                logger.warning(f"FlagCX communicator creation failed: {e}, using torch.distributed")
                self._flagcx_comm = None
    ...
    def all_gatherv(
        ...
        if self._flagcx_comm and not self._flagcx_comm.disabled:
            output_list = []
            self._flagcx_comm.flagcx.flagcxGroupStart() if hasattr(self._flagcx_comm, 'flagcx') else None
            for inp in input_:
                output_list.append(_all_gather_single(inp, sizes=sizes))
            self._flagcx_comm.flagcx.flagcxGroupEnd() if hasattr(self._flagcx_comm, 'flagcx') else None
            return output_list
        else:
            output_list = []
            for inp in input_:
